...
protected:
  ...
  virtual bool parseString(std::string value){
    if (!value.empty() && value != ""){
      ...
      JsonVariant _var = _buffer.parseObject(value.c_str());
...
public:
  ...
  inline bool remove(){ return doLog<bool>("nodeConfig: remove not implemented", false); };
  inline virtual queryResponse* execute(nodeQuery* query){ doLog<std::string>("Query not implemented", ""); };
